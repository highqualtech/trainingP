...
@section('content')
    ...
    @if($courseslide->audiofile!='')
        <div class="alert alert-dark">
            Click on the Play button below to listen to the audio file for this slide:<br>
            <figure>
                <audio
                    controls
                    src="/slideaudio/{{ $courseslide->audiofile }}">
    ...
    @if($courseslide->slide_type=='1')
        {!!  $courseslide->slidehtml !!}
        <form method="POST" action="{{ route('participantcourse.slideprocess', ['coursekey' => $coursekey,'slidekey' => $courseslide->slide_key]) }}">
            @csrf
            @method('POST')<br><br>
            <input type="submit" name="submit" value="NEXT SLIDE" class="btn btn-success float-end">
        </form>
    @elseif($courseslide->slide_type=='2')
    ...
    @elseif(($courseslide->slide_type=='3')||($courseslide->slide_type=='5')||($courseslide->slide_type=='6'))
        @if($courseslide->slide_type=='5'){!!  $courseslide->slidehtml !!}@endif
        @if($courseslide->slide_type=='6')<img src="/slideimage/{{ $courseslide->slide_image }}" class="img-fluid">@endif
        ...
        @endif
    @elseif($courseslide->slide_type=='4')
        {!!  $courseslide->slidehtml  !!}
        <form method="POST" action="{{ route('participantcourse.slideprocess', ['coursekey' => $coursekey,'slidekey' => $courseslide->slide_key]) }}">
            @csrf
            @method('POST')
        ...
        <script type="text/javascript">
            ...
            $(".submitanswers").click(function(){
                ...
                @endif
                    ...
                    $.ajax({
                        url: "/c_submit/{{ $coursekey}}/{{ $courseslide->slide_key }}",
                        type: "post",
                        data: {
                            _token: CSRF_TOKENX,
                        ...
                        success: function (response) {
                            ...
                            if (response.status == 'success') {
                                ...
                                $("#answerresponse").html("Your answer is correct. You will now be redirected to the next question.");
                                  setTimeout(function() {
                                      window.location.href = "/c/{{ $coursekey}}/{{ $courseslide->slide_key }}";
                                   }, 3000);
                                @endif
